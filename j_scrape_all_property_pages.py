...
def scrape_all_property_pages(source_file, target_path):
    ...
    with requests.Session() as session:
        with ThreadPoolExecutor(max_workers=10) as executor:
            property_data = list(executor.map(scrape_data_from_property_page, [link[1] for link in links_rimanenti], [link[0] for link in links_rimanenti],[session] * len(links_rimanenti),[target_path] * len(links_rimanenti)))
...
scrape_all_property_pages("links.json", "3.output.jsonl")
...
def scrape_all_property_pages(source_file, target_path):
    ...
    with requests.Session() as session:
        with ThreadPoolExecutor(max_workers=10) as executor:
            property_data = list(executor.map(scrape_data_from_property_page, links_as_dict.values(), links_as_dict.keys(),[session] * len(links_as_dict),[target_path] * len(links_as_dict)))
        for item in property_data:
            if item:
                final_list.append(item)
